...
export async function main(fixtureArg = process.argv[2] ?? DEFAULT_FIXTURE) {
  ...
  try {
    ...
    const user = await clerk.users.createUser({
      externalId: `greenlight-pipeline-${nonce}`,
      skipPasswordRequirement: true,
      skipLegalChecks: true,
      privateMetadata: { purpose: "ephemeral-pipeline-smoke" },
    });
    userId = user.id;
    const session = await clerk.sessions.createSession({ userId });
    sessionId = session.id;
    const token = (await clerk.sessions.getToken(session.id, undefined, 600)).jwt;

    const caseKey = `pipeline-case-${nonce}`;
    const created = await api(apiBase, token, "/api/cases", {
      method: "POST",
    ...
    const reserved = await api(apiBase, token, "/api/uploads", {
      method: "POST",
    ...
    const completed = await api(apiBase, token, `/api/uploads/${documentId}/complete`, { method: "POST" }, [200]);
    const completedStatus = string(data(completed.body, "upload completion").status, "upload completion status");
    const analyzed = await api(apiBase, token, `/api/documents/${documentId}/analyze`, { method: "POST" }, [200]);
    ...
    for (const field of fields) {
      await api(apiBase, token, `/api/documents/${documentId}/fields`, {
        method: "PATCH",
    ...
    const evaluated = await api(apiBase, token, `/api/cases/${caseId}/evaluate`, { method: "POST" }, [200]);
    const evaluation = data(evaluated.body, "evaluation");
    const resultResponse = await api(apiBase, token, `/api/cases/${caseId}/result`, { method: "GET" }, [200]);
    ...
    if (eligible) {
      ...
      const prepared = await api(apiBase, token, `/api/opportunities/${opportunityId}/prepare`, {
        method: "POST",
        headers: { "idempotency-key": `pipeline-prepare-${nonce}` },
      }, [200, 201]);
      const actionId = string(data(prepared.body, "prepared action").id, "actionId");
      await api(apiBase, token, `/api/actions/${actionId}/approve`, {
        method: "POST",
      ...
      const blocked = await api(apiBase, token, `/api/opportunities/${opportunityId}/prepare`, {
        method: "POST",
    ...
    if (sessionId) await clerk.sessions.revokeSession(sessionId).catch(() => undefined);
